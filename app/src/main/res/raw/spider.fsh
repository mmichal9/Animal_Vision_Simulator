...
float pixelIntensity(vec2 uv, vec2 d) {
	vec3 pix = texture2D(iChannel0, uv + d / iResolution.xy).rgb;
...
vec3 colormix(vec3 d ){
	...
	//Intensity Matrix
	vec3 c_r = vec3(.8, 0.2, 0.2);
	vec3 c_g = vec3(0., 1.2, -0.2);
	vec3 c_b = vec3(0.1, 0.2, .7);
